...
## Acceptance criteria
- [x] Wrapper retrieves/creates a 32-byte KEK from macOS Keychain (`service=ocprotectfs`, `account=kek`) and uses it for FUSE.
- [x] Wrapper passes the KEK to `ocprotectfs-fuse` via an anonymous pipe (FD) and **does not** set `OCPROTECTFS_KEK_B64`.
- [x] `ocprotectfs-fuse` supports `--kek-fd <n>` (reads exactly 32 bytes) and prefers it over env.
- [x] Docs updated:
  - remove/soft-deprecate env-based KEK in operator docs,
  - describe the FD handoff contract and Keychain location.
- [x] `npm test` passes locally.
- [x] `tasks/STATUS.md` updated.
